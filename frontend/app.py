...
# Check if we need to fetch the data
    ...
    with st.sidebar:
        ...
        # Only render the timeline UI if we successfully got data from the backend
        if timeline:
            st.divider()
            st.subheader("🤖 AI Clinical Summary")
            # st.info creates a nice highlighted blue box for the AI summary
            st.info(timeline.get("summary", "Summary unavailable."))
            
            st.divider()
            st.subheader("📈 Recent Visits")
            # Extract the top 3 visits dynamically
            recent_visits = timeline.get("recent_visits", [])
            if recent_visits:
                # Format options for the radio button: "YYYY-MM-DD - Department"
                options = [f"{v['date_recorded'][:10]} - {v['department']}" for v in recent_visits]
                st.radio("Select Visit Context", options)
            else:
                st.caption("No recent visits.")
                
            # Use an expander for older visits to keep the UI clean
            older_visits = timeline.get("older_visits", [])
            if older_visits:
                with st.expander("📂 Older Patient File (Complete History)"):
                    for ov in older_visits:
                        st.markdown(f"**{ov['date_recorded'][:10]}**<br/>*{ov['department']}*", unsafe_allow_html=True)
                        st.caption(f"{ov['note_content'][:50]}...")
                        st.divider()
    ...
    with tab_analytics:
        ...
        if timeline and "analytics" in timeline:
            stats = timeline.get("analytics", {})
            
            if not stats:
                st.info("No AI-extracted data available for this patient yet. Add a new note to trigger extraction!")
            else:
                # 1. High-Level Metrics
                col1, col2, col3 = st.columns(3)
                total_visits = len(timeline.get("recent_visits", [])) + len(timeline.get("older_visits", []))
                total_tags = len(stats)
                
                col1.metric(label="Total Logged Visits", value=total_visits)
                col2.metric(label="Tracked Medical Entities", value=total_tags)
                col3.metric(label="Risk Level", value="Requires Review" if total_visits > 3 else "Stable")
                
                st.divider()
                
                # 2. DYNAMIC PARSING: Group all tags by their AI-generated Category
                dynamic_categories = {}
                for tag, count in stats.items():
                    # Check if the tag has the correct "Category: Value" format
                    if ": " in tag:
                        category, value = tag.split(": ", 1)
                        # Add the category to our dictionary if it doesn't exist yet
                        if category not in dynamic_categories:
                            dynamic_categories[category] = []
                        # Add the value to that category
                        dynamic_categories[category].append(value)
                
                # 3. DYNAMIC RENDERING: Draw the UI based on whatever the AI found
                category_names = list(dynamic_categories.keys())
                
                if not category_names:
                    st.warning("Data format error. Could not parse AI tags.")
                else:
                    # Create 2 columns for a clean grid layout
                    cols = st.columns(2)
                    
                    # Loop through every category the AI found and draw it on the screen
                    for i, category in enumerate(category_names):
                        # Use modulo arithmetic to alternate placing sections in the left (0) or right (1) column
                        with cols[i % 2]:
                            st.markdown(f"### 📌 {category}")
                            for val in dynamic_categories[category]:
                                st.markdown(f"- {val}")
